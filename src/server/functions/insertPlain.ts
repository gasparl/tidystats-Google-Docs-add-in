export const insertPlain = (textToInsert) => {
    ...
    if (cursor) {
        ...
        const cElement: any = cursor.insertText(textToInsert);
        if (!cElement) {
            DocumentApp.getUi().alert('Cannot insert text here.');
        } else {
            doc.setCursor(doc.newPosition(cElement, textToInsert.length));
        }
    } else {
        ...
        if (!selection) {
            ...
            for (let i = 0; i < elements.length; i++) {
                if (elements[i].isPartial()) {
                    const tElement: any = elements[i].getElement().asText();
                    ...
                    if (replace) {
                        ...
                        replace = false;
                        doc.setCursor(doc.newPosition(tElement, textToInsert.length));
                    }
                } else {
